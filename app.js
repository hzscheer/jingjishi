...
const App = {
  ...
  // 将字母答案(A/B/C/D/ADE等)转换为可比较的格式
  // 返回 { indices: [0,3,4], isMulti: true/false }
  answerToIndices(answer) {
    if (!answer) return { indices: [], isMulti: false };
    const letters = answer.trim().toUpperCase();
    const indices = [];
    for (const ch of letters) {
      const idx = ch.charCodeAt(0) - 65; // 'A'=65
      if (idx >= 0 && idx < 26) indices.push(idx);
    }
    return { indices, isMulti: indices.length > 1 };
  },

  // 判断用户选择是否正确
  isAnswerCorrect(userAns, correctAnswer) {
    const { indices } = this.answerToIndices(correctAnswer);
    return indices.includes(userAns);
  },

  // 获取正确答案的显示文本
  getCorrectAnswerText(options, correctAnswer) {
    const { indices } = this.answerToIndices(correctAnswer);
    return indices.map(i => options[i] || '').join('；');
  },
  ...
  renderStage1() {
    ...
    if (chapters.length === 0) {
      html += `<div class="empty-state"><div class="empty-icon">📭</div><div class="empty-text">暂无考点数据</div></div>`;
    } else {
      chapters.forEach((ch, ci) => {
        const mastered = ch.points.filter(p => p.mastered).length;
        // 统计本章母题完成情况
        const mqAnswers = JSON.parse(localStorage.getItem('mq_answers') || '{}');
        const mqCh = (d.motherQuestions[this.currentSubject]?.chapters || []).find(
          mc => mc.name.includes(ch.name.substring(0, 4))
        );
        let mqDoneCount = 0, mqTotalCount = 0;
        if (mqCh) {
          mqTotalCount = mqCh.questions.length;
          mqDoneCount = mqCh.questions.filter(q => mqAnswers[q.id] !== undefined).length;
        }
        ...
        // 一章一练按钮
        html += `<div style="text-align:center;padding:8px;border-top:1px solid var(--border);">
          <button class="btn btn-primary btn-sm" onclick="App.startChapterPractice('${this.currentSubject}','${ch.id}')">
            一章一练 (${mqDoneCount}/${mqTotalCount}已完成)
          </button>
        </div></div></div>`;
      });
    }
  ...
  // 一章一练 - 从考点页面开始的章节练习
  startChapterPractice(subject, chId) {
    // 找到对应的母题章节
    const chName = (this.data.points[subject]?.chapters || []).find(c => c.id === chId)?.name || '';
    const mqChapters = this.data.motherQuestions[subject]?.chapters || [];
    const mqCh = mqChapters.find(mc => mc.name.includes(chName.substring(0, 4)));
    
    if (mqCh) {
      this.currentSubject = subject;
      this.startMQPractice(subject, mqCh.id);
    } else {
      this.toast('该章节暂无练习题');
    }
  },
  ...
  // 获取某个考点关联的母题数量
  getPointQuestionCount(pointTitle, chapterName, subject) {
    const mqChapters = this.data.motherQuestions[subject]?.chapters || [];
    const mqCh = mqChapters.find(mc => mc.name.includes(chapterName.substring(0, 4)));
    if (!mqCh) return 0;
    return mqCh.questions.length;
  },
  ...
  showPointDetail(subject, chId, pId) {
    ...
    const mqChapters = this.data.motherQuestions[subject]?.chapters || [];
    const mqCh = mqChapters.find(mc => mc.name.includes(chName.substring(0, 4)));
    const relatedQuestions = mqCh ? mqCh.questions : [];
    const qCount = relatedQuestions.length;
    ...
    let html = `<div class="detail-overlay" onclick="if(event.target===this)App.closeDetail()">
      <div class="detail-sheet">
        ...
        <div class="detail-content">
          <div style="margin-bottom:8px;"><strong>所属章节：</strong>${chName}</div>
          <div style="margin-bottom:8px;"><strong>关联题目：</strong>${qCount} 道母题</div>
        </div>
    ...
    if (qCount > 0) {
      html += `<div style="text-align:center;margin-bottom:12px;">
        <button class="btn btn-primary btn-sm" onclick="App.closeDetail();App.startChapterPractice('${subject}','${chId}')">开始本章练习</button>
      </div>`;
    }
  ...
  renderStage2() {
    ...
    if (chapters.length === 0) {
      html += `<div class="empty-state"><div class="empty-icon">📭</div><div class="empty-text">暂无母题数据</div></div>`;
    } else {
      chapters.forEach((ch, ci) => {
        ...
        ch.questions.forEach((q, qi) => {
          ...
          const isCorrect = isDone && this.isAnswerCorrect(userAns, q.answer);
          html += `<div class="point-item">
  ...
  startMQPractice(subject, chId) {
    ...
    html += `<div class="paper-header"><div class="paper-title">${ch.name} - 母题</div><div class="paper-meta">共 ${ch.questions.length} 题</div></div>`;
    
    ch.questions.forEach((q, qi) => {
      const userAns = answers[q.id];
      const answered = userAns !== undefined;
      const isCorrect = answered && this.isAnswerCorrect(userAns, q.answer);
      const correctIndices = this.answerToIndices(q.answer);
      
      html += `<div class="quiz-card" id="mq-card-${q.id}">
        <div class="quiz-num">第 ${qi+1}/${ch.questions.length} 题 <span style="color:var(--text-muted);font-size:var(--font-size-xs);">${q.type==='multi'?'[多选题]':'[单选题]'}</span></div>
        <div class="quiz-question">${q.question}</div>`;
      
      q.options.forEach((opt, oi) => {
        let cls = 'quiz-option';
        if (answered) {
          if (correctIndices.indices.includes(oi)) cls += ' correct';
      ...
      if (answered) {
        html += `<div class="quiz-answer">
          ...
          <div style="margin-top:6px;"><strong>正确答案：</strong>${this.getCorrectAnswerText(q.options, q.answer)}</div>
          <div style="margin-top:8px;"><strong>解析：</strong>${q.analysis || '暂无解析'}</div>`;
  ...
  answerMQ(qId, userAns) {
    ...
    const isCorrect = question && this.isAnswerCorrect(userAns, question.answer);
    ...
    this.toast(isCorrect ? '回答正确！' : '回答错误，已加入错题集');
    this.data = DataStore.load();
  ...
  renderStage3() {
    ...
    if (years.length === 0) {
      html += `<div class="empty-state"><div class="empty-icon">📭</div><div class="empty-text">暂无真题数据，可在"我的"页面导入真题JSON</div></div>`;
    } else {
    ...
    paper.questions.forEach((q, qi) => {
      ...
      if (answered) {
        html += `<div class="quiz-answer">
          ...
          <div style="margin-top:6px;"><strong>正确答案：</strong>${this.getCorrectAnswerText(q.options, q.answer)}</div>
          <div style="margin-top:8px;"><strong>解析：</strong>${q.analysis || '暂无解析'}</div></div>`;
      }
      html += `</div>`;
    });
  ...
  answerExam(qId, userAns) {
    ...
    this.toast(isCorrect ? '回答正确！' : '回答错误');
    this.data = DataStore.load();
  ...
  showExamResult(subject, paperId) {
    ...
    paper.questions.forEach(q => {
      ...
      else if (this.isAnswerCorrect(ua, q.answer)) correct++;
      else wrong++;
    });
    ...
    if (wrong > 0) {
      ...
      paper.questions.forEach(q => {
        const ua = answers[q.id];
        if (ua !== undefined && !this.isAnswerCorrect(ua, q.answer)) {
          html += `<div style="font-size:var(--font-size-sm);margin-bottom:8px;padding:8px;background:var(--danger-bg);border-radius:6px;">
            <div><strong>${q.question}</strong></div>
            <div style="color:var(--danger);">你的答案: ${q.options[ua] || '未知'}</div>
            <div style="color:var(--success);">正确答案: ${this.getCorrectAnswerText(q.options, q.answer)}</div>
          </div>`;
        }
      });
      html += `</div>`;
      ...
      html += `<div class="card" style="padding:12px;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">
          <div style="flex:1;">
            ...
            <div style="font-size:var(--font-size-sm);color:var(--danger);">你的答案: ${(w.options && w.options[w.userAnswer]) || '未知'}</div>
            <div style="font-size:var(--font-size-sm);color:var(--success);">正确答案: ${this.getCorrectAnswerText(w.options || [], w.answer)}</div>
          </div>
        </div>
  ...
  filterWrong(filter, el) {
    ...
    if (filter !== 'all') {
      if (filter === '经济基础知识' || filter === '人力资源管理专业知识和实务') {
  ...
  showWrongDetail(wrongId) {
    ...
    let html = `<div class="detail-overlay" onclick="if(event.target===this)App.closeDetail()">
      <div class="detail-sheet">
        ...
        <div style="margin-bottom:8px;"><span style="color:var(--danger);">你的答案：${(w.options && w.options[w.userAnswer]) || '未知'}</span></div>
        <div style="margin-bottom:8px;"><span style="color:var(--success);">正确答案：${this.getCorrectAnswerText(w.options || [], w.answer)}</span></div>
        <div class="detail-section"><div class="detail-section-content"><strong>解析：</strong>${w.analysis || '暂无解析'}</div></div>`;
  ...
  submitRetry(wrongId, userAns) {
    ...
    const isCorrect = this.isAnswerCorrect(userAns, w.answer);
    const correctIndices = this.answerToIndices(w.answer);
    const resultDiv = document.getElementById(`retry-result-${wrongId}`);
    
    document.querySelectorAll('#modalContent .quiz-option').forEach(b => {
      b.disabled = true;
      const optIdx = Array.from(b.parentElement.querySelectorAll('.quiz-option')).indexOf(b);
      if (correctIndices.indices.includes(optIdx)) b.classList.add('correct');
    ...
    if (isCorrect) {
      ...
      resultDiv.innerHTML = `<div class="quiz-answer"><span class="wrong-text">还是错了</span><div>正确答案：${this.getCorrectAnswerText(w.options, w.answer)}</div></div>`;
    }
  ...
  showRandomResult() {
    ...
    questions.forEach((q, qi) => {
      ...
      const card = document.getElementById(`rnd-card-${qi}`);
      if (!card || ua === undefined) return;
      
      const opts = card.querySelectorAll('.quiz-option');
      const isCorrect = this.isAnswerCorrect(ua, q.answer);
      const correctIndices = this.answerToIndices(q.answer);
      ...
      if (isCorrect) {
        ...
        correctIndices.indices.forEach(i => {
          document.getElementById(`rnd-opt-${qi}-${i}`)?.classList.add('correct');
        });
      ...
      let rHtml = `<div class="quiz-answer">
        <span class="${isCorrect?'correct-text':'wrong-text'}">${isCorrect?'正确':'错误'}</span>
        <div style="margin-top:6px;"><strong>正确答案：</strong>${this.getCorrectAnswerText(q.options, q.answer)}</div>
        <div style="margin-top:6px;"><strong>解析：</strong>${q.analysis || '暂无解析'}</div>`;
  ...
  renderCheckin() {
    ...
    const stage2Start = new Date(2026, 8, 1);
    const stage3Start = new Date(2026, 9, 1);
  ...
  renderMe() {
    ...
    // 按考点维度统计掌握情况
    let pointStats = this.computePointStats(mqAnswers);
    ...
    let html = `<div class="fade-in">
      ...
      <div class="card">
        <div class="card-title" style="margin-bottom:12px;">考点维度掌握情况</div>`;
    
    for (const subject of ['经济基础知识', '人力资源管理专业知识和实务']) {
      const shortName = subject === '经济基础知识' ? '经济基础' : '人力资源';
      html += `<div style="margin-bottom:12px;">
        <div style="font-weight:600;margin-bottom:6px;">${shortName}</div>`;
      
      const subStats = pointStats[subject] || [];
      if (subStats.length === 0) {
        html += `<div style="font-size:var(--font-size-xs);color:var(--text-muted);">暂无数据</div>`;
      } else {
        subStats.forEach(ps => {
          const pct = ps.total > 0 ? Math.round((ps.correct / ps.total) * 100) : 0;
          const color = pct >= 80 ? 'var(--success)' : pct >= 60 ? 'var(--warning)' : 'var(--danger)';
          html += `<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 0;font-size:var(--font-size-xs);">
            <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${ps.name}</span>
            <span style="color:${color};font-weight:600;margin-left:8px;">${pct}%</span>
            <span style="color:var(--text-muted);margin-left:8px;">(${ps.correct}/${ps.total})</span>
          </div>`;
        });
      }
      html += `</div>`;
    ...
    html += `</div>
      <div class="card">
        ...
        <div class="settings-item">
          ...
          <div class="switch ${this.data.settings.darkMode?'on':''}" onclick="App.toggleDarkMode()"></div>
        </div>
        <div class="settings-item">
    ...
    for (const subject of ['经济基础知识', '人力资源管理专业知识和实务']) {
      ...
      chapters.forEach(ch => {
        ...
        ch.questions.forEach(q => {
          const ua = mqAnswers[q.id];
          if (ua !== undefined) {
            total++;
            if (this.isAnswerCorrect(ua, q.answer)) correct++;
          }
        });
        if (total > 0) {
          stats[subject].push({ name: ch.name, correct, total });
        }
      });
    }
    return stats;
  ...
  exportData() {
    ...
    a.href = url;
    a.download = 'exam_prep_backup.json';
    a.click();
  ...
  showQuizDetail(category, subject, chId, qId) {
    ...
    const correctIndices = this.answerToIndices(question.answer);
    ...
    question.options.forEach((opt, oi) => {
      ...
      if (userAns !== undefined) {
        if (correctIndices.indices.includes(oi)) style = 'color:var(--success);font-weight:600;';
        else if (oi === userAns && !correctIndices.indices.includes(oi)) style = 'color:var(--danger);font-weight:600;';
      }
      html += `<div style="padding:6px 0;${style}">${opt}${correctIndices.indices.includes(oi)?' ✓':''}</div>`;
    });
    
    html += `<div class="detail-section"><div class="detail-section-content"><strong>解析：</strong>${question.analysis || '暂无解析'}</div></div>`;
